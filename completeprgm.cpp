...
void torso()
{
	...
	gluCylinder(t, TORSO_RADIUS / 2.0, TORSO_RADIUS, TORSO_HEIGHT, 20, 10);
	glPopMatrix();
}
...
void head()
{
	...
	gluCylinder(h, 0.9*HEAD_RADIUS, 0.9*HEAD_RADIUS, HEAD_HEIGHT / 8, 10, 10);
	glPopMatrix();
...
void idle()
{
	...
	case 0:
		...
		else if (!done)
		{
			...
			glLoadIdentity();
			glTranslatef(0, 0, 0.0);
			glRotatef(theta[5], 1.0, 1.0, 1.0);
			glGetFloatv(GL_MODELVIEW_MATRIX, rua_node.m);

			glLoadIdentity();
			glTranslatef(0, 0, 0.0);
			glRotatef(theta[3], 1.0, 1.0, 1.0);
			glGetFloatv(GL_MODELVIEW_MATRIX, lua_node.m);
...
int main(int argc, char **argv)
{
	...
	glutCreateWindow(" Robot simple movements");
	myinit();
